...
export function SelfTest() {
  ...
  return <section className="self-test-panel self-test-question" aria-labelledby="self-test-question"><div className="self-test-top"><span className="eyebrow">快速自测</span><span>{progress}</span></div><div className="self-test-progress" role="progressbar" aria-label="快速自测进度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progressPercent} aria-valuetext={progress}><span style={{ width: `${progressPercent}%` }} /></div><h2 id="self-test-question">{exercise.prompt}</h2><div className="self-test-display">{isTrigramSignature(exercise.display) ? <TrigramSignatureVisual value={exercise.display} /> : exercise.display}</div><div className="self-test-choices">{exercise.choices.map((option) => <button type="button" key={option} className={selected === option ? option === exercise.answer ? "correct" : "incorrect" : ""} onClick={() => void choose(option)} disabled={selected !== null || saving}>{isTrigramSignature(option) ? <TrigramSignatureVisual value={option} /> : option}</button>)}</div>{selected !== null && <div className={`self-test-feedback ${selected === exercise.answer ? "is-correct" : "is-wrong"}`} role="status" aria-live="polite"><strong>{selected === exercise.answer ? "回答正确" : <>{"答案是："}{isTrigramSignature(exercise.answer) ? <TrigramSignatureVisual value={exercise.answer} /> : exercise.answer}</>}</strong><span>{exercise.explanation}</span></div>}{selected !== null && index < exercises.length - 1 && <button type="button" className="next-review" onClick={next}>下一题 <span>→</span></button>}{saving && <small className="self-test-saving" role="status" aria-live="polite">正在保存自测结果…</small>}</section>;
}
