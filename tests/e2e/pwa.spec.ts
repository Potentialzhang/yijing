...
test("生产环境离线打开实验室分享链接时保留查询参数", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(async () => {
        const cache = await caches.open("yijing-static-v70");
...
test("生产环境升级 Service Worker 时会清理旧版本缓存", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    const v67PreviousCache = await caches.open("yijing-static-v67");
    await v67PreviousCache.put(
      "/immediately-previous-cache-marker-v67",
      new Response("immediately-previous-v67", { headers: { "content-type": "text/plain" } }),
    );
    const v68PreviousCache = await caches.open("yijing-static-v68");
    await v68PreviousCache.put(
      "/immediately-previous-cache-marker-v68",
      new Response("immediately-previous-v68", { headers: { "content-type": "text/plain" } }),
    );
    const v69PreviousCache = await caches.open("yijing-static-v69");
    await v69PreviousCache.put(
      "/immediately-previous-cache-marker-v69",
      new Response("immediately-previous-v69", { headers: { "content-type": "text/plain" } }),
    );
  ...
  await expect
    .poll(() => page.evaluate(() => caches.keys()))
    .not.toContain("yijing-static-v67");
  await expect
    .poll(() => page.evaluate(() => caches.keys()))
    .not.toContain("yijing-static-v68");
  await expect
    .poll(() => page.evaluate(() => caches.keys()))
    .not.toContain("yijing-static-v69");
  await expect
    .poll(() => page.evaluate(() => caches.keys()))
