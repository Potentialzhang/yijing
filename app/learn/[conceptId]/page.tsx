...
export default async function ConceptDetailPage({ params }: { params: Promise<{ conceptId: string }> }) {
  ...
  const nextLabel = conceptId === "hetu-luoshu" || conceptId === "nine-palaces"
    ...
    : nextConcept
      ...
      : "进入复习队列";
  return <main className="subpage"><header className="subpage-header"><Link href="/learn" className="back-link">← 知识路径</Link><p className="eyebrow">{concept.stage}</p><h1>{concept.title}</h1><div className="detail-header-actions"><p className="subpage-lead">{concept.summary}</p><FavoriteButton targetType="concept" targetId={concept.id} /></div></header><PrerequisiteNotice prerequisites={prerequisites.filter((item): item is (typeof KNOWLEDGE_CONCEPTS)[number] => Boolean(item)).map((item) => ({ id: item.id, title: item.title }))} /><LessonProgress conceptId={concept.id} /><section className="lesson-detail-body"><div className="lesson-detail-main"><h2>核心理解</h2>{concept.blocks.map((block, index) => <article className="content-block" key={block.id}><h3>{block.title ?? (index === 0 ? "先掌握" : "再理解")}</h3><MarkdownPreview markdown={block.markdown} /></article>)}<h2>记忆抓手</h2><div className="lesson-keywords">{concept.keywords.map((keyword) => <span key={keyword}>{keyword}</span>)}</div>{concept.commonConfusions?.length ? <><h2>容易混淆</h2><ul className="confusion-list">{concept.commonConfusions.map((item) => <li key={item}>{item}</li>)}</ul></> : null}<details className="source-status-details"><summary>查看本节内容来源</summary><div className="source-status-list">{sources.map((source) => <div key={source!.id}><strong>{source!.title}</strong><span className={source!.status === "verified" ? "source-verified" : "source-pending"}>{source!.status === "verified" ? "已登记" : "待校对"}</span></div>)}</div></details></div><aside className="lesson-detail-side" aria-label="学习关系"><span className="content-label">学习导航</span><h2>前置知识</h2>{prerequisites.length ? prerequisites.map((item) => <Link href={`/learn/${item!.id}`} key={item!.id}>{item!.title} ↗</Link>) : <p>这是当前知识路径的起点。</p>}<h2>相关知识</h2>{relatedConcepts.length ? relatedConcepts.map((item) => <Link href={`/learn/${item.id}`} key={item.id}>{item.title} ↗</Link>) : <p>暂无相关知识。</p>}<h2>下一步</h2><Link href={nextHref}>{nextLabel} ↗</Link></aside></section><div className="test-academy-link" aria-label="统一测试入口"><span>读完后先合上页面复述，再用题库验证。</span><Link className="outline-button" href="/test-academy">进入测试学堂 <span>↗</span></Link></div><ContentErrata targetType="concept" targetId={concept.id} contentVersion={concept.contentVersion ?? 1} /><NoteEntry targetType="concept" targetId={concept.id} label="打开本节个人笔记" /></main>;
}
