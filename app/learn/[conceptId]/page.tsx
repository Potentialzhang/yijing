...
export default async function ConceptDetailPage({ params }: { params: Promise<{ conceptId: string }> }) {
  ...
  const nextLabel = conceptId === "hetu-luoshu" || conceptId === "nine-palaces"
    ...
    : nextConcept
      ...
      : "进入复习队列";
  return <main className="subpage"><header className="subpage-header"><Link href="/learn" className="back-link">← 学习地图</Link><p className="eyebrow">{concept.stage}</p><h1>{concept.title}</h1><div className="detail-header-actions"><p className="subpage-lead">{concept.summary}</p><FavoriteButton targetType="concept" targetId={concept.id} /></div></header><PrerequisiteNotice prerequisites={prerequisites.filter((item): item is (typeof KNOWLEDGE_CONCEPTS)[number] => Boolean(item)).map((item) => ({ id: item.id, title: item.title }))} /><LessonProgress conceptId={concept.id} /><section className="lesson-detail-body"><div className="lesson-detail-main"><span className="content-label">编辑释义 · 入门</span>{concept.blocks.map((block) => <div className="content-block" key={block.id}><span className="content-block-kind">{block.kind === "editorial" ? "编辑释义" : block.kind} · {block.traditionTags.join("、")}</span><span className="content-block-source">来源：{block.sourceIds.map((sourceId) => SOURCE_REGISTRY.find((source) => source.id === sourceId)?.title ?? sourceId).join("、")}</span><MarkdownPreview markdown={block.markdown} /></div>)}<h2>本节关键词</h2><div className="lesson-keywords">{concept.keywords.map((keyword) => <span key={keyword}>{keyword}</span>)}</div>{concept.commonConfusions?.length ? <><h2>容易混淆</h2><ul className="confusion-list">{concept.commonConfusions.map((item) => <li key={item}>{item}</li>)}</ul></> : null}<h2>来源状态</h2><div className="source-status-list">{sources.map((source) => <div key={source!.id}><strong>{source!.title}</strong><span className={source!.status === "verified" ? "source-verified" : "source-pending"}>{source!.status === "verified" ? "已登记" : "待校对"}</span></div>)}</div></div><aside className="lesson-detail-side" aria-label="学习关系"><span className="content-label">学习关系</span><h2>前置知识</h2>{prerequisites.length ? prerequisites.map((item) => <Link href={`/learn/${item!.id}`} key={item!.id}>{item!.title} ↗</Link>) : <p>这是当前学习地图的起点。</p>}<h2>相关知识</h2>{relatedConcepts.length ? relatedConcepts.map((item) => <Link href={`/learn/${item.id}`} key={item.id}>{item.title} ↗</Link>) : <p>暂无已登记的相关知识。</p>}<h2>下一步</h2><Link href={nextHref}>{nextLabel} ↗</Link></aside></section><InstantPractice targetType="concept" targetId={concept.id} /><ContentErrata targetType="concept" targetId={concept.id} contentVersion={concept.contentVersion ?? 1} /><NoteEditor targetType="concept" targetId={concept.id} /></main>;
}
