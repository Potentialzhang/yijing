...
export function AccountGate({ children }: { children: React.ReactNode }) {
  ...
  if (state === "signed-out") return <main className="subpage account-required"><span className="eyebrow">需要账户</span><h1>先登录，再保存你的学习记录。</h1><p className="subpage-lead">公开的卦象和学习内容仍可浏览；笔记、复习、收藏、偏好和 AI 草稿只保存到你的 PostgreSQL 账户数据库。</p><Link className="primary-button" href="/account">登录或注册</Link></main>;
  return <>{children}</>;
}
