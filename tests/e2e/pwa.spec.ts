...
test("生产环境离线打开实验室分享链接时保留查询参数", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(async () => {
        const cache = await caches.open("yijing-static-v62");
...
test("生产环境升级 Service Worker 时会清理旧版本缓存", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    const latestPreviousCache = await caches.open("yijing-static-v61");
  ...
  await expect
    .poll(() => page.evaluate(() => caches.keys()))
    .not.toContain("yijing-static-v61");
  await expect
    .poll(() => page.evaluate(() => caches.keys()))
