...
function lineDescription(lines: readonly (0 | 1)[]) {
...
export const EXERCISES: readonly Exercise[] = [
  ...
  ...TRIGRAMS.map((trigram) => ({ id: `trigram-name-lines-${trigram.id}`, kind: "trigram-name-lines" as const, prompt: `按卦名选择${trigram.name}的三爻结构`, targetType: "trigram" as const, targetId: trigram.id, display: trigram.name, choices: choices(trigram.lines.join(""), TRIGRAMS.map((item) => item.lines.join(""))), answer: trigram.lines.join(""), explanation: `${trigram.name}的正确结构是${lineDescription(trigram.lines)}。` })),
  ...
  ...TRIGRAMS.map((trigram) => ({ id: `trigram-arrange-lines-${trigram.id}`, kind: "trigram-arrange-lines" as const, prompt: `按从下往上排列${trigram.name}的三爻`, targetType: "trigram" as const, targetId: trigram.id, display: trigram.name, choices: ["阳爻", "阴爻"], answer: trigram.lines.join(""), explanation: `${trigram.name}的正确排列是${lineDescription(trigram.lines)}。` })),
